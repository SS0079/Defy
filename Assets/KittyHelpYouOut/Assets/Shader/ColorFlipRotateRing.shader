Shader "KittyShader/ColorFlipRotateRingUnlit"
{
    Properties
    {
        _Diffuse ("Diffuse", Color) = (1,1,1,1)
        _Round("Round",Range(-4,4))=0
        _RingStart("RingStart",float)=0.1
        _RingWidth("RingWidth",float)=0.1
        _RingSpace("RingSpace",float)=0.05
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float thisDis=length(float2(i.objPos.x,i.objPos.y));
                if (_Round>0)//clockwise
                {
                    ...
                    if (thisDis>_RingStart)//check if beyond ring start
                    {
